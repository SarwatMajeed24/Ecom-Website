...
//     setTotalPrice((prevTotal) => prevTotal - foundProduct.price * foundProduct.quatity);
//     setTotalQuantity((prevTotalQty) => prevTotalQty - foundProduct.quatity)
...
const onRemove = (product: any) => {
    ...
    if (foundProduct) {
      ...
      // Fallback to ensure no NaN issues
      setTotalPrice((prevTotal) => Math.max(0, prevTotal));
      setTotalQuantity((prevTotalQty) => Math.max(0, prevTotalQty));
    }
  };
    ...
    return(
        ...
        value = {{showCart, setShowCart, qty, incQty, decQty, cartItems, addProduct, totalQuantity, totalPrice, toggleCartItemQty , onRemove}}
        >
            <div>{children}</div>
        </CartContext.Provider>
    )
